...
func NewCompanyHandler(cfg *config.Config, configUC *usecase.ConfigUsecase, bots whatsapp.BotProvider) *CompanyHandler {
...
func (h *CompanyHandler) getSettings(c *fiber.Ctx) error {
	...
	return c.JSON(fiber.Map{
		...
		"data": fiber.Map{
			...
			"whatsapp_phone":      company.WhatsappPhone.String,
			"auth_status":         company.AuthStatus.String,
			"subscription_status": company.SubscriptionStatus.String,
			"subscription_expiry": company.SubscriptionExpiry.Time,
			"plan_type":           company.PlanType.String,
			"brand_color":         company.BrandColor.String,
			"logo_url":            company.LogoUrl.String,
		},
	})
}
...
func (h *CompanyHandler) updateSettings(c *fiber.Ctx) error {
	...
	err = h.configUC.UpdateCompanySettings(c.Context(), companyID, req.Name, req.AdminEmail, req.LogoUrl)
	if err != nil {
		logger.Error().Err(err).Msg("Failed to update company settings")
...
func (h *CompanyHandler) resendSetupLink(c *fiber.Ctx) error {
	...
	go notif.SendSetupLinkEmail(h.cfg, company.AdminEmail, company.Name, newToken)
