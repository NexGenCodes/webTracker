            ...
            {registerStep === 'credentials' && (
                <form onSubmit={form.handleSubmit(onRegisterIntent)} className="space-y-5">
                    <div className="flex flex-col items-center mb-8 text-center">
                        ...
                        <h1 className="text-3xl font-black text-text-main tracking-tighter uppercase mb-2">Get Started</h1>
                        <p className="text-text-muted font-bold text-sm uppercase tracking-widest opacity-70">Create your account</p>
                    </div>
            ...
            {registerStep === 'otp' && (
                <form onSubmit={handleVerifyRegisterOTP} className="space-y-8">
                    <div className="flex flex-col items-center mb-8 text-center">
                        ...
                        <h1 className="text-3xl font-black text-text-main tracking-tighter uppercase mb-2">Verify Email</h1>
                        <p className="text-text-muted font-bold text-sm tracking-wide opacity-70">We sent a 6-digit code to</p>
                    ...
                    <button type="submit" disabled={loading || otp.join('').length < 6} className="btn-primary w-full py-4 text-base flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50 transition-all duration-200">
                        {loading ? <Loader2 className="animate-spin" size={20} /> : <>Verify & Continue <ArrowRight size={18} /></>}
                    </button>

                    <p className="text-xs text-text-muted text-center flex flex-col gap-2 items-center">
                        {otpTimer > 0 ? (
                            <span>Code expires in <span className="font-bold text-accent">{formatTime(otpTimer)}</span></span>
                        ) : (
                            <span className="text-error font-bold">Code expired</span>
                        )}
                        <span>
                            Didn&apos;t get the code?{' '}
                            <button 
