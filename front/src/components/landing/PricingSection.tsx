        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {BILLING_PLANS.map((plan, i) => {
            ...
            if (!isCustom) {
                ...
                if (isYearly) {
                    const yearlyMonthly = numericPrice * 0.85;
                    priceDisplay = `₦${yearlyMonthly.toLocaleString()}`;
                    yearlyBilled = `₦${(yearlyMonthly * 12).toLocaleString()}`;
                }
            }
            ...
            return (
              ...
              >
                ...
                <div className="mb-2">
                  {isCustom ? (
                    <span className="text-4xl md:text-5xl font-black">{dict.marketing?.pricing?.custom || 'Custom'}</span>
                  ) : (
                    <>
